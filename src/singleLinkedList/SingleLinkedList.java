...
public class SingleLinkedList {
    ...
    public void addNode(int value) {
        if(this.head == null) {
            ...
            node.next = new singleNode(value);
        }
    }

    public void insertNode(singleNode findNode, int value) {
        //삽입할 노드의 다음 노드를 nextNode 라는 곳에 저장
        singleNode nextNode = findNode.next;

        //조회한 노드 다음 노드에 새로운 값 적어주고 그곳을 node라는 곳으로 받음
        findNode.next = new singleNode(value);
        singleNode node = findNode.next;

        //받은 node의 next값에 이전에 저장해둔 nextNode를 넣어줌
        node.next = nextNode;
    }

    public void deleteNode(singleNode prevNode) {
        //삭제할 다음 노드 찾아서 저장하기
        singleNode nextNode = prevNode.next.next;
